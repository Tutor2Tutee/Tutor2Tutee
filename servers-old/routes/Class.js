...
router.get('/', (req, res) => {
    Class.find()
        .populate('teacher', '_id nickname email')
        .exec((err, res1) => {
            if (err) {
                res.status(409).json({
                    success: false,
...
router.post('/', (req, res) => {
    ...
    Class.findByName(name).then(create).then(respond).catch(onError);
});
...
router.put('/:id', (req, res) => {
    ...
    const checkTeacher = (_class) => {
        if (_class.teacher.toString() === teacherID) return _class;
        else throw new Error('teacher');
    };
    ...
    const change = async (_class) => {
        ...
        if (point && _class.point !== point) {
            await Class.updateOne({ _id: req.params.id }, { $set: { point } });
            changes.push('point');
        }
...
router.delete('/:id', (req, res) => {
    ...
    const deleteClass = (_class) => {
        Class.deleteOne({ _id: _class._id }).then(() => {
            return res.status(200).json({
                success: true,
